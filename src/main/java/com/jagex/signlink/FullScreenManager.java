...
public class FullScreenManager {
        ...
        if (this.windowedDisplayMode == null) {
            ...
            if (refreshRate == 0) {
                ...
                for (DisplayMode displayMode : this.fullScreenSupportedGraphicsDevice.getDisplayModes()) {
                    if (displayMode.getWidth() == width && height == displayMode.getHeight() && bitDepth == displayMode.getBitDepth()) {
                        int candidateRefreshRate = displayMode.getRefreshRate();

                        if (!foundBetterRefreshRate || Math.abs(candidateRefreshRate - windowedRefreshRate) < Math.abs(windowedRefreshRate - refreshRate)) {
            ...
            this.fullScreenSupportedGraphicsDevice.setDisplayMode(new DisplayMode(width, height, bitDepth, refreshRate));
        }
    }
    ...
    public void exitFullScreen() {
        if (this.windowedDisplayMode != null) {
            ...
            if (!this.fullScreenSupportedGraphicsDevice.getDisplayMode().equals(this.windowedDisplayMode)) {
