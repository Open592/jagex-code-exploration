...
public final class SignLink implements Runnable {
  ...
  private static void bumpAWTThreadPriority() {
    ...
    for (ThreadGroup parent = threadGroup.getParent();
        parent != null;
  ...
  private static File resolveCacheFilePath(
      String filename, int modewhat, String cacheSubDirectoryName) {
    ...
    String[] potentialParentDirectories =
        new String[] {
          "c:/rscache/", "/rscache/", "c:/windows/", "c:/winnt/", "c:/", systemUserHome, "/tmp/", ""
        };
    String[] potentialCacheDirectories =
        ...
        for (String potentialParentDirectory : potentialParentDirectories) {
          String fullFilePath =
              potentialParentDirectory
                  + potentialCacheDirectory
                  + "/"
                  + (cacheSubDirectoryName == null ? "" : cacheSubDirectoryName + "/")
                  + filename;
          ...
          try {
            ...
            if (cacheFindPass != 0 || filePath.exists()) {
              if (cacheFindPass != 1
                  || potentialParentDirectory.isEmpty()
  ...
  private static FileOnDisk resolvePreferencesFileLocation(
      String id, String gameName, int modewhat) {
    String filename;
    ...
    String[] potentialParentDirectories =
        new String[] {
          "c:/rscache/", "/rscache/", systemUserHome, "c:/windows/", "c:/winnt", "c:/", "/tmp/", "",
        };
  ...
  public SignLink(Applet hostApplet, int modewhat, String gameName, int cacheArchiveCount)
      throws Exception {
    ...
    try {
      if (hostApplet != null) {
        setFocusTraversalKeysEnabled =
            hostApplet.getClass().getMethod("setFocusTraversalKeysEnabled", Boolean.TYPE);
      } else {
        setFocusTraversalKeysEnabled =
      ...
      if (hostApplet != null) {
        ...
        setFocusCycleRoot =
            Class.forName("java.awt.Container")
                .getDeclaredMethod("setFocusCycleRoot", Boolean.TYPE);
      }
    } catch (Exception ignored) {
    }

    this.randomFile =
        new FileOnDisk(resolveCacheFilePath("random.dat", this.modewhat, null), "rw", 25L);
    this.cacheDataFile =
        new FileOnDisk(this.resolveCacheFilePath("main_file_cache.dat2"), "rw", 209715200L);
    this.cacheIndex255 =
        new FileOnDisk(this.resolveCacheFilePath("main_file_cache.idx255"), "rw", 1048576L);
    this.cacheIndexFiles = new FileOnDisk[cacheArchiveCount];

    for (int i = 0; i < cacheArchiveCount; i++) {
      this.cacheIndexFiles[i] =
          new FileOnDisk(this.resolveCacheFilePath("main_file_cache.idx" + i), "rw", 1048576L);
    }
  ...
  public Message emitEnterFullScreenModeMessage(int arg0, int arg1, int arg2) {
    return this.enqueueMessage(
        Message.ENTER_FULLSCREEN_MODE_MESSAGE, (arg2 << 16) + arg0, arg1 << 16, null);
  }
  ...
  public Message emitGetDeclaredMethodMessage(
      Class<?> clazz, String methodName, Class<?>[] parameterTypes) {
    return this.enqueueMessage(
        Message.GET_DECLARED_METHOD_MESSAGE,
        0,
        0,
        new Object[] {clazz, methodName, parameterTypes});
  }

  public Message emitGetDeclaredFieldMessage(Class<?> clazz, String fieldName) {
    return this.enqueueMessage(
        Message.GET_DECLARED_FIELD_MESSAGE, 0, 0, new Object[] {clazz, fieldName});
  }

  public Message emitPerformClassLoaderNativeLibrariesCleanupMessage(Class<?> fromClass) {
    return this.enqueueMessage(
        Message.PERFORM_CLASS_LOADER_NATIVE_LIBRARIES_CLEANUP_MESSAGE, 0, 0, fromClass);
  }
  ...
  public Message emitSetCustomCursorMessage(
      Component component, int width, int height, int[] rgbArray, Point hotSpot) {
    return this.enqueueMessage(
        Message.SET_CUSTOM_CURSOR_MESSAGE,
        width,
        height,
        new Object[] {component, rgbArray, hotSpot});
  }
  ...
  public void loadGlNatives(Class<?> fromClass) throws Exception {
    ...
    Method setAccessibleMethod =
        Class.forName("java.lang.reflect.Method").getMethod("setAccessible", Boolean.TYPE);
    Method loadMethod;

    if (!systemOSNameLowerCase.startsWith("mac")) {
      loadMethod =
          Class.forName("java.lang.Runtime")
              .getDeclaredMethod("loadLibrary0", Class.class, String.class);
    ...
    loadMethod =
        Class.forName("java.lang.Runtime").getDeclaredMethod("load0", Class.class, String.class);
  ...
  private Message enqueueMessage(
      int type, int firstIntegerInput, int secondIntegerInput, Object genericInput) {
  ...
  private Message emitCustomCursorMouseMoveMessage(Component component, int x, int y) {
    ...
    return this.enqueueMessage(
        Message.CUSTOM_CURSOR_MOUSE_MOVE_MESSAGE, x + local7.x, local7.y + y, null);
  }

  private Message emitSetCustomCursorComponentMessage(Component component, boolean shouldReset) {
    return this.enqueueMessage(
        Message.SET_CUSTOM_CURSOR_COMPONENT_MESSAGE, shouldReset ? 1 : 0, 0, component);
  }
  ...
  private void performConnectionInitialization(Message message) throws IOException {
    ...
    message.output =
        new Socket(InetAddress.getByName((String) message.genericInput), message.firstIntegerInput);
  }
  ...
  private void performReverseIPLookup(Message message) throws IOException {
    ...
    String host =
        ((message.firstIntegerInput >> 24) & 255)
            + "."
  ...
  private void performGetDeclaredMethod(Message message) throws NoSuchMethodException {
    ...
    message.output =
        ((Class<?>) input[0]).getDeclaredMethod((String) input[1], (Class<?>[]) input[2]);
  }
  ...
  private void performClassLoaderNativeLibrariesCleanup(Message message)
    ...
    Field nativeLibrariesField =
        Class.forName("java.lang.ClassLoader").getDeclaredField("nativeLibraries");

    nativeLibrariesField.setAccessible(true);

    Vector<?> nativeLibraries =
        (Vector<?>) nativeLibrariesField.get(((Class<?>) message.genericInput).getClassLoader());
  ...
  private void performResolvePreferencesFileLocation(Message message) {
    message.output =
        resolvePreferencesFileLocation((String) message.genericInput, gameName, modewhat);
  }
    ...
    try {
      ...
      if (systemOSName.startsWith("win")) {
        if (!systemOSArch.startsWith("amd64") && !systemOSArch.startsWith("x86_64")) {
          load0Method.invoke(
              runtime, message.genericInput, resolveCacheFilePath("jagmisc.dll").toString());
        } else {
          load0Method.invoke(
              runtime, message.genericInput, resolveCacheFilePath("jagmisc64.dll").toString());
        }
      }
