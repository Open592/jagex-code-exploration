...
public final class ServerConnection implements Runnable {
  ...
  @OriginalMember(
      owner = "client!al",
      name = "<init>",
      descriptor = "(Ljava/net/Socket;Lclient!et;)V")
  public ServerConnection(@OriginalArg(0) Socket connection, @OriginalArg(1) SignLink signLink)
      throws IOException {
  ...
  public void readBytesFromServer(
      @OriginalArg(0) int offset, @OriginalArg(2) int length, @OriginalArg(3) byte[] buffer)
      throws IOException {
    ...
    while (length > 0) {
      @Pc(23)
      int bytesRead = this.inputStream.read(buffer, offset, length);
