...
public final class FullScreenMode {
    ...
    public static Frame enterFullScreenMode(int width, SignLink signLink, int height, int bitDepth) {
        ...
        for (GraphicsDeviceDisplayMode displayMode : displayModes) {
            if (displayMode.width == width && displayMode.height == height && (!foundBitDepth || displayMode.bitDepth > bitDepth)) {
                foundBitDepth = true;
        ...
        Message enterFullScreenModeMessage = signLink.emitEnterFullScreenModeMessage(height, bitDepth, width);
