...
public final class Static421 {
	...
	public static String method5388(@OriginalArg(1) int arg0) {
		...
		if (local13.length() > 9) {
			return " <col=00ff80>" + local13.substring(0, local13.length() - 8) + Static452.A_LOCALIZED_STRING___152.getLocalizedString(ClientSettings.langID) + " (" + local13 + ")</col>";
		} else if (local13.length() > 6) {
			return " <col=ffffff>" + local13.substring(0, local13.length() - 4) + Static214.A_LOCALIZED_STRING___79.getLocalizedString(ClientSettings.langID) + " (" + local13 + ")</col>";
		} else {
