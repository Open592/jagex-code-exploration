...
public enum ModeGame {
  ...
  public static Optional<ModeGame> fromGameId(String input) {
     ...
     * <p>"game0" or "0"
     *
     * <p>We try to deduce the format before resolving the GameWhat value
     */
    String gameId =
        input.startsWith("game") ? String.valueOf(input.charAt(input.length() - 1)) : input;

    switch (gameId) {
      case "0":
        return Optional.of(ModeGame.RUNESCAPE);
      case "1":
        return Optional.of(ModeGame.STELLAR_DAWN);
      default:
