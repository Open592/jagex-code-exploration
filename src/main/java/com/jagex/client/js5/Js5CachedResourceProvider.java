package com.jagex.client.js5;

import com.jagex.client.HashMap;
import com.jagex.client.Node;
import com.jagex.client.ds.LinkedList;
import com.jagex.signlink.MonotonicClock;
import java.util.zip.CRC32;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!lv")
public final class Js5CachedResourceProvider extends Js5ResourceProvider {

  @OriginalMember(owner = "client!qj", name = "n", descriptor = "Ljava/util/zip/CRC32;")
  public static final CRC32 aCRC32_1 = new CRC32();

  @OriginalMember(owner = "client!lv", name = "n", descriptor = "[B")
  private byte[] aByteArray56;

  @OriginalMember(owner = "client!lv", name = "t", descriptor = "Lclient!rt;")
  private Js5Index index;

  @OriginalMember(owner = "client!lv", name = "F", descriptor = "Z")
  private boolean prefetchAll;

  @OriginalMember(owner = "client!lv", name = "w", descriptor = "I")
  private int anInt4482 = 0;

  @OriginalMember(owner = "client!lv", name = "o", descriptor = "Lclient!ib;")
  private final HashMap groupRequests = new HashMap(16);

  @OriginalMember(owner = "client!lv", name = "J", descriptor = "I")
  private int anInt4489 = 0;

  @OriginalMember(owner = "client!lv", name = "G", descriptor = "Lclient!pk;")
  private final LinkedList prefetchQueue = new LinkedList();

  @OriginalMember(owner = "client!lv", name = "L", descriptor = "J")
  private long aLong149 = 0L;

  @OriginalMember(owner = "client!lv", name = "h", descriptor = "Lclient!st;")
  private final Cache groupCache;

  @OriginalMember(owner = "client!lv", name = "j", descriptor = "I")
  private final int archive;

  @OriginalMember(owner = "client!lv", name = "D", descriptor = "Z")
  private boolean groupCacheInitialized;

  @OriginalMember(owner = "client!lv", name = "H", descriptor = "Lclient!pk;")
  private LinkedList groupQueue;

  @OriginalMember(owner = "client!lv", name = "g", descriptor = "I")
  private final int checksum;

  @OriginalMember(owner = "client!lv", name = "k", descriptor = "I")
  private final int version;

  @OriginalMember(owner = "client!lv", name = "K", descriptor = "Z")
  private final boolean aBoolean414;

  @OriginalMember(owner = "client!lv", name = "i", descriptor = "Lclient!st;")
  private final Cache archiveCache;

  @OriginalMember(owner = "client!lv", name = "l", descriptor = "Lclient!vn;")
  private final Js5NetQueue js5NetQueue;

  @OriginalMember(owner = "client!lv", name = "B", descriptor = "Lclient!vl;")
  private final Js5DiskCache diskCache;

  @OriginalMember(owner = "client!lv", name = "x", descriptor = "Lclient!ec;")
  private Js5QueueRequest request;

  @OriginalMember(
      owner = "client!lv",
      name = "<init>",
      descriptor = "(ILclient!st;Lclient!st;Lclient!vn;Lclient!vl;IIZ)V")
  public Js5CachedResourceProvider(
      int archive,
      Cache arg1,
      Cache arg2,
      Js5NetQueue js5NetQueue,
      Js5DiskCache arg4,
      int checksum,
      int version,
      boolean arg7) {
    this.groupCache = arg1;
    this.archive = archive;

    if (this.groupCache == null) {
      this.groupCacheInitialized = false;
    } else {
      this.groupCacheInitialized = true;
      this.groupQueue = new LinkedList();
    }

    this.checksum = checksum;
    this.version = version;
    this.aBoolean414 = arg7;
    this.archiveCache = arg2;
    this.js5NetQueue = js5NetQueue;
    this.diskCache = arg4;

    if (this.archiveCache != null) {
      this.request = this.diskCache.method5432(this.archiveCache, this.archive);
    }
  }

  @OriginalMember(owner = "client!lv", name = "b", descriptor = "(B)I")
  public int method3517() {
    return this.index == null ? 0 : this.index.size;
  }

  @OriginalMember(owner = "client!lv", name = "a", descriptor = "(III)Lclient!ec;")
  private Js5QueueRequest method3518(int arg0, int group) {
    Js5QueueRequest groupRequest = (Js5QueueRequest) this.groupRequests.get(group);

    if (groupRequest != null
        && arg0 == 0
        && !groupRequest.isUrgent
        && groupRequest.isRequestInProgress) {
      groupRequest.popSelf();
      groupRequest = null;
    }

    if (groupRequest == null) {
      if (arg0 == 0) {
        if (this.groupCache == null || this.aByteArray56[group] == -1) {
          if (this.js5NetQueue.isUrgentRequestQueueFull()) {
            return null;
          }

          groupRequest = this.js5NetQueue.requestArchiveFile(this.archive, (byte) 2, true, group);
        } else {
          groupRequest = this.diskCache.method5432(this.groupCache, group);
        }
      } else if (arg0 == 1) {
        if (this.groupCache == null) {
          throw new RuntimeException();
        }

        groupRequest = this.diskCache.method5431(this.groupCache, group);
      } else if (arg0 == 2) {
        if (this.groupCache == null) {
          throw new RuntimeException();
        }

        if (this.aByteArray56[group] != -1) {
          throw new RuntimeException();
        }

        if (this.js5NetQueue.isRegularRequestQueueFull()) {
          return null;
        }

        groupRequest = this.js5NetQueue.requestArchiveFile(this.archive, (byte) 2, false, group);
      } else {
        throw new RuntimeException();
      }

      this.groupRequests.set(group, groupRequest);
    }

    if (groupRequest.isRequestInProgress) {
      return null;
    }

    byte[] local161 = groupRequest.getResponseData();
    int local188;
    Js5NetQueueRequest local238;

    if (!(groupRequest instanceof Js5DiskCacheRequestItem)) {
      try {
        if (local161 == null || local161.length <= 2) {
          throw new RuntimeException();
        }

        aCRC32_1.reset();
        aCRC32_1.update(local161, 0, local161.length - 2);

        local188 = (int) aCRC32_1.getValue();

        if (local188 != this.index.anIntArray430[group]) {
          throw new RuntimeException();
        }

        this.js5NetQueue.connectionFailures = 0;
        this.js5NetQueue.errorCode = 0;
      } catch (RuntimeException e) {
        this.js5NetQueue.method5464();

        groupRequest.popSelf();

        if (groupRequest.isUrgent && !this.js5NetQueue.isUrgentRequestQueueFull()) {
          local238 = this.js5NetQueue.requestArchiveFile(this.archive, (byte) 2, true, group);
          this.groupRequests.set(group, local238);
        }

        return null;
      }

      local161[local161.length - 2] = (byte) (this.index.anIntArray429[group] >>> 8);
      local161[local161.length - 1] = (byte) this.index.anIntArray429[group];

      if (this.groupCache != null) {
        this.diskCache.method5435(group, local161, this.groupCache);

        if (this.aByteArray56[group] != 1) {
          this.anInt4482++;
          this.aByteArray56[group] = 1;
        }
      }

      if (!groupRequest.isUrgent) {
        groupRequest.popSelf();
      }

      return groupRequest;
    }
    try {
      if (local161 == null || local161.length <= 2) {
        throw new RuntimeException();
      }

      aCRC32_1.reset();
      aCRC32_1.update(local161, 0, local161.length - 2);
      local188 = (int) aCRC32_1.getValue();

      if (this.index.anIntArray430[group] != local188) {
        throw new RuntimeException();
      }

      int local371 =
          ((local161[local161.length - 2] & 0xFF) << 8) + (local161[local161.length - 1] & 0xFF);

      if (local371 != (this.index.anIntArray429[group] & 0xFFFF)) {
        throw new RuntimeException();
      }

      if (this.aByteArray56[group] != 1) {
        this.anInt4482++;
        this.aByteArray56[group] = 1;
      }

      if (!groupRequest.isUrgent) {
        groupRequest.popSelf();
      }

      return groupRequest;
    } catch (Exception e) {
      this.aByteArray56[group] = -1;

      groupRequest.popSelf();

      if (groupRequest.isUrgent && !this.js5NetQueue.isUrgentRequestQueueFull()) {
        local238 = this.js5NetQueue.requestArchiveFile(this.archive, (byte) 2, true, group);

        this.groupRequests.set(group, local238);
      }

      return null;
    }
  }

  @OriginalMember(owner = "client!lv", name = "b", descriptor = "(II)[B")
  @Override
  public byte[] method3516(int arg0) {
    Js5QueueRequest local16 = this.method3518(0, arg0);

    if (local16 == null) {
      return null;
    } else {
      byte[] local24 = local16.getResponseData();

      local16.popSelf();

      return local24;
    }
  }

  @OriginalMember(owner = "client!lv", name = "d", descriptor = "(I)V")
  public void method3520() {
    if (this.groupQueue != null) {
      if (this.fetchIndex() == null) {
        return;
      }

      boolean local21;
      Node local28;
      int local34;
      Node local127;

      if (this.groupCacheInitialized) {
        local21 = true;

        for (local28 = this.groupQueue.tail();
            local28 != null;
            local28 = this.groupQueue.previous()) {
          local34 = (int) local28.hashKey;
          if (this.aByteArray56[local34] == 0) {
            this.method3518(1, local34);
          }
          if (this.aByteArray56[local34] == 0) {
            local21 = false;
          } else {
            local28.popSelf();
          }
        }

        while (this.anInt4489 < this.index.anIntArray428.length) {
          if (this.index.anIntArray428[this.anInt4489] == 0) {
            this.anInt4489++;
          } else {
            if (this.diskCache.anInt7015 >= 250) {
              local21 = false;
              break;
            }
            if (this.aByteArray56[this.anInt4489] == 0) {
              this.method3518(1, this.anInt4489);
            }
            if (this.aByteArray56[this.anInt4489] == 0) {
              local127 = new Node();
              local127.hashKey = this.anInt4489;
              local21 = false;
              this.groupQueue.addFirst(local127);
            }
            this.anInt4489++;
          }
        }
        if (local21) {
          this.anInt4489 = 0;
          this.groupCacheInitialized = false;
        }
      } else if (this.prefetchAll) {
        local21 = true;
        for (local28 = this.groupQueue.tail();
            local28 != null;
            local28 = this.groupQueue.previous()) {
          local34 = (int) local28.hashKey;

          if (this.aByteArray56[local34] != 1) {
            this.method3518(2, local34);
          }

          if (this.aByteArray56[local34] == 1) {
            local28.popSelf();
          } else {
            local21 = false;
          }
        }

        while (this.anInt4489 < this.index.anIntArray428.length) {
          if (this.index.anIntArray428[this.anInt4489] == 0) {
            this.anInt4489++;
          } else {
            if (this.js5NetQueue.isRegularRequestQueueFull()) {
              local21 = false;
              break;
            }

            if (this.aByteArray56[this.anInt4489] != 1) {
              this.method3518(2, this.anInt4489);
            }

            if (this.aByteArray56[this.anInt4489] != 1) {
              local127 = new Node();
              local127.hashKey = this.anInt4489;
              local21 = false;
              this.groupQueue.addFirst(local127);
            }

            this.anInt4489++;
          }
        }

        if (local21) {
          this.anInt4489 = 0;
          this.prefetchAll = false;
        }
      } else {
        this.groupQueue = null;
      }
    }
    if (!this.aBoolean414 || this.aLong149 > MonotonicClock.getCurrentTimeInMilliseconds()) {
      return;
    }

    for (Js5QueueRequest request = (Js5QueueRequest) this.groupRequests.head();
        request != null;
        request = (Js5QueueRequest) this.groupRequests.next()) {
      if (!request.isRequestInProgress) {
        if (request.isRequested) {
          if (!request.isUrgent) {
            throw new RuntimeException();
          }

          request.popSelf();
        } else {
          request.isRequested = true;
        }
      }
    }

    this.aLong149 = MonotonicClock.getCurrentTimeInMilliseconds() + 1000L;
  }

  @OriginalMember(owner = "client!lv", name = "a", descriptor = "(Z)I")
  public int method3521() {
    if (this.index == null) {
      return 0;
    } else if (this.groupCacheInitialized) {
      Node local25 = this.groupQueue.tail();

      return local25 == null ? 0 : (int) local25.hashKey;
    } else {
      return this.index.size;
    }
  }

  @OriginalMember(owner = "client!lv", name = "a", descriptor = "(IB)I")
  @Override
  public int getDownloadPercentage(int arg0) {
    Js5QueueRequest local11 = (Js5QueueRequest) this.groupRequests.get(arg0);

    return local11 == null ? 0 : local11.getDownloadPercentage();
  }

  @OriginalMember(owner = "client!lv", name = "c", descriptor = "(B)V")
  public void method3522() {
    if (this.groupQueue == null || this.fetchIndex() == null) {
      return;
    }

    for (Node local23 = this.prefetchQueue.tail();
         local23 != null;
         local23 = this.prefetchQueue.previous()) {
      int local29 = (int) local23.hashKey;

      if (local29 < 0 || local29 >= this.index.capacity || this.index.anIntArray428[local29] == 0) {
        local23.popSelf();
      } else {
        if (this.aByteArray56[local29] == 0) {
          this.method3518(1, local29);
        }
        if (this.aByteArray56[local29] == -1) {
          this.method3518(2, local29);
        }
        if (this.aByteArray56[local29] == 1) {
          local23.popSelf();
        }
      }
    }
  }

  @OriginalMember(owner = "client!lv", name = "f", descriptor = "(I)I")
  public int getDownloadPercentage() {
    if (this.fetchIndex() == null) {
      return this.request == null ? 0 : this.request.getDownloadPercentage();
    } else {
      return 100;
    }
  }

  @OriginalMember(owner = "client!lv", name = "a", descriptor = "(II)V")
  @Override
  public void method3512(int arg0) {
    if (this.groupCache == null) {
      return;
    }

    for (Node local14 = this.prefetchQueue.tail();
         local14 != null;
         local14 = this.prefetchQueue.previous()) {
      if (local14.hashKey == (long) arg0) {
        return;
      }
    }

    Node local43 = new Node();

    local43.hashKey = arg0;

    this.prefetchQueue.addFirst(local43);
  }

  @OriginalMember(owner = "client!lv", name = "d", descriptor = "(B)V")
  public void prefetchAll() {
    if (this.groupCache != null) {
      this.prefetchAll = true;

      if (this.groupQueue == null) {
        this.groupQueue = new LinkedList();
      }
    }
  }

  @OriginalMember(owner = "client!lv", name = "g", descriptor = "(I)I")
  public int method3527() {
    return this.anInt4482;
  }

  @OriginalMember(owner = "client!lv", name = "a", descriptor = "(B)Lclient!rt;")
  @Override
  public Js5Index fetchIndex() {
    if (this.index != null) {
      return this.index;
    }

    if (this.request == null) {
      if (this.js5NetQueue.isUrgentRequestQueueFull()) {
        return null;
      }

      this.request = this.js5NetQueue.requestArchiveFile(255, (byte) 0, true, this.archive);
    }

    if (this.request.isRequestInProgress) {
      return null;
    }

    byte[] responseData = this.request.getResponseData();

    if (this.request instanceof Js5DiskCacheRequestItem) {
      try {
        if (responseData == null) {
          throw new RuntimeException();
        }

        this.index = new Js5Index(responseData, this.checksum);

        if (this.index.version != this.version) {
          throw new RuntimeException();
        }
      } catch (RuntimeException e) {
        this.index = null;

        if (this.js5NetQueue.isUrgentRequestQueueFull()) {
          this.request = null;
        } else {
          this.request = this.js5NetQueue.requestArchiveFile(255, (byte) 0, true, this.archive);
        }

        return null;
      }
    } else {
      try {
        if (responseData == null) {
          throw new RuntimeException();
        }

        this.index = new Js5Index(responseData, this.checksum);
      } catch (RuntimeException e) {
        this.js5NetQueue.method5464();
        this.index = null;

        if (this.js5NetQueue.isUrgentRequestQueueFull()) {
          this.request = null;
        } else {
          this.request = this.js5NetQueue.requestArchiveFile(255, (byte) 0, true, this.archive);
        }

        return null;
      }

      if (this.archiveCache != null) {
        this.diskCache.method5435(this.archive, responseData, this.archiveCache);
      }
    }

    this.request = null;

    if (this.groupCache != null) {
      this.aByteArray56 = new byte[this.index.capacity];
      this.anInt4482 = 0;
    }

    return this.index;
  }
}
