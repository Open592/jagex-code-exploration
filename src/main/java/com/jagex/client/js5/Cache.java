...
public final class Cache {
	...
	public Cache(int archiveNumber, BufferedFile cacheDataFile, BufferedFile cacheIndexFile, int maxLength) {
	...
	public byte[] read(int group) {
		synchronized (this.cacheDataFile) {
			try {
				...
				int length = (Static166.aByteArray37[2] & 0xFF) + (((Static166.aByteArray37[0] & 0xFF) << 16) + ((Static166.aByteArray37[1] & 0xFF) << 8));
				int block = ((Static166.aByteArray37[3] & 0xFF) << 16) + ((Static166.aByteArray37[4] & 0xFF) << 8) + (Static166.aByteArray37[5] & 0xFF);
			...
			try {
				...
				while (arg2 > local163) {
					...
					if (arg3) {
						...
						int local242 = ((Static166.aByteArray37[2] & 0xFF) << 8) + (Static166.aByteArray37[3] & 0xFF);
